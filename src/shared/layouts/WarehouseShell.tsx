...
import { Archive, Boxes, ClipboardList, LayoutDashboard, PackagePlus } from 'lucide-react';
...
export const WarehouseShell: React.FC<{children:React.ReactNode}> = ({children}) => {
 ...
 useEffect(()=>{const refresh=()=>warehouseService.dashboard().then(x=>setCounts({inbound:x.pendingReceipt,putaway:x.awaitingPutaway,inventory:x.availableQuantity})).catch(()=>{});refresh();const id=window.setInterval(refresh,10000);return()=>window.clearInterval(id);},[location.pathname]);
