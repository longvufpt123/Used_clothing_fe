...
const orderNearest=(start:Point,points:Point[])=>{
  const pending=[...points],result:Point[]=[]; let current=start;
  while(pending.length){let best=0;for(let i=1;i<pending.length;i++)if(distance(current,pending[i])<distance(current,pending[best]))best=i;
    current=pending.splice(best,1)[0];result.push(current);}
  return result;
};
const asciiAddress=(value:string)=>value.normalize('NFD').replace(/[\u0300-\u036f]/g,'')
  .replace(/[đĐ]/g,m=>m==='đ'?'d':'D')
  .replace(/\b(Phuong|TP\.?|Thanh pho)\b/gi,'').replace(/\s+/g,' ').trim();
...
async function geocode(address:string,fallback?:{lat:number;lon:number}):Promise<Point>{
  const key=`osm:${address.toLowerCase()}`,cached=localStorage.getItem(key);
  ...
  for(let i=0;i<queries.length;i++){
    ...
    if(data[0]){
      ...
      localStorage.setItem(key,JSON.stringify(point)); return point;
    }
  }
  if(fallback){
    const point={...fallback,address,label:address}; localStorage.setItem(key,JSON.stringify(point)); return point;
  }
...
export default function RouteMap({batch}:{batch:ReceivingBatch}){
  ...
  const build=async()=>{
    ...
    try{
      const warehouse={...(await geocode(batch.warehouseAddress,{lat:10.8514649,lon:106.7711441})),label:'Kho xuất phát'};
      const addresses=[...new Map(batch.requests.filter(r=>r.deliveryMethod==='StaffPickup')
        .map(r=>[r.pickupAddress,r])).entries()];
      const pickupPoints:Point[]=[];
      const skipped:string[]=[];
      for(const [address,request] of addresses){
        await wait(1100);
        try{pickupPoints.push({...await geocode(address),label:request.donorName,requestId:request.id});}
        catch{skipped.push(address);}
      }
      const ordered=orderNearest(warehouse,pickupPoints),all=[warehouse,...ordered];
      setPoints(all);
      if(all.length>1){
        const coords=all.map(p=>`${p.lon},${p.lat}`).join(';');
        const res=await fetch(`https://router.project-osrm.org/route/v1/driving/${coords}?overview=full&geometries=geojson`);
        const json=await res.json();
        setRoute((json.routes?.[0]?.geometry?.coordinates||[]).map((x:[number,number])=>[x[1],x[0]]));
  ...
  return <div className="route-map-wrap">
    ...
    <MapContainer bounds={bounds} scrollWheelZoom style={{height:420,width:'100%'}}>
      ...
      {route.length>1&&<Polyline positions={route} pathOptions={{color:'#059669',weight:5}}/>}
      {points.map((p,i)=><Marker key={`${p.address}-${i}`} position={[p.lat,p.lon]}
        zIndexOffset={i===0?1000:0} icon={marker(String(i),i===0?'#0f172a':'#059669')}>
        <Popup><b>{i===0?'Kho xuất phát':`Điểm ${i}: ${p.label}`}</b><br/>{p.address}</Popup>
      </Marker>)}
    </MapContainer>
    <small>Dữ liệu bản đồ © OpenStreetMap · Tuyến đường OSRM · Thứ tự là gợi ý, staff có thể điều chỉnh theo thực tế.</small>
  </div>;
}
