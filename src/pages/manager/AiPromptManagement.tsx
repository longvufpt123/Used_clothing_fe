...
export default function AiPromptManagement() {
  ...
  const [form,setForm]=useState({name:'',promptText:'',enabled:true,dailyRequestLimit:'',totalRequestLimit:''});
  const [usingDefault,setUsingDefault]=useState(true); const [updatedAt,setUpdatedAt]=useState<string>();
  const [saving,setSaving]=useState(false);
  const load=async()=>{try{const x=await aiPromptService.getClassification();setForm({name:x.name,promptText:x.promptText,enabled:x.enabled,dailyRequestLimit:x.dailyRequestLimit!=null?String(x.dailyRequestLimit):'',totalRequestLimit:x.totalRequestLimit!=null?String(x.totalRequestLimit):''});setUsingDefault(x.isUsingDefault);setUpdatedAt(x.updatedAt);}catch{toast.error('Không tải được cấu hình prompt AI.');}};
  useEffect(()=>{void load();},[]);
  const save=async()=>{
    if(!form.name.trim()||!form.promptText.trim())return toast.warning('Tên và nội dung prompt không được để trống.');
    const daily=form.dailyRequestLimit.trim()?Number(form.dailyRequestLimit):null;
    const total=form.totalRequestLimit.trim()?Number(form.totalRequestLimit):null;
    if(daily!=null&&(!Number.isFinite(daily)||daily<1))return toast.warning('Giới hạn mỗi ngày phải là số nguyên dương.');
    if(total!=null&&(!Number.isFinite(total)||total<1))return toast.warning('Giới hạn tổng phải là số nguyên dương.');
    setSaving(true);try{await aiPromptService.saveClassification({...form,dailyRequestLimit:daily,totalRequestLimit:total});toast.success('Đã cập nhật prompt phân loại AI.');await load();}catch(e:any){toast.error(e?.response?.data?.message||'Không lưu được prompt.');}finally{setSaving(false);}};
  ...
  return <div className="ops-page ai-prompt-page"><header className="ops-pagehead"><div><span className="ops-pagehead-kicker"><Sparkles/> AI Configuration</span><h1>Quản lý prompt phân loại</h1><p>Điều chỉnh hướng dẫn nghiệp vụ mà Gemini sử dụng khi phân tích hình ảnh quần áo.</p></div><span className={`ops-badge ${form.enabled?'done':'pending'}`}>{form.enabled?'Đang áp dụng':'Đang tắt'}</span></header>
    <div className="ai-prompt-layout"><section className="ops-panel glass ai-prompt-editor"><div className="ai-prompt-section-head"><Bot/><div><h2>Prompt đang sử dụng</h2><p>{usingDefault?'Đang dùng cấu hình mặc định':'Đang dùng cấu hình do Manager thiết lập'}{updatedAt?` · cập nhật ${new Date(updatedAt).toLocaleString('vi-VN')}`:''}</p></div></div>
      ...
      <div className="ai-prompt-quota-fields">
        <div className="ops-field"><label>Giới hạn số lần dùng AI mỗi ngày / user</label><input type="number" min={1} placeholder="Không giới hạn" value={form.dailyRequestLimit} onChange={e=>setForm({...form,dailyRequestLimit:e.target.value})}/></div>
        <div className="ops-field"><label>Giới hạn tổng số lần dùng AI / user</label><input type="number" min={1} placeholder="Không giới hạn" value={form.totalRequestLimit} onChange={e=>setForm({...form,totalRequestLimit:e.target.value})}/></div>
      </div>
