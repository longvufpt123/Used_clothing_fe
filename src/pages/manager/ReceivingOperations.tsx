...
const batchStatus: Record<string, string> = {
  Planned: 'Đã lập lô',
  Receiving: 'Đang thu gom',
  Completed: 'Đã gom xong',
...
export default function ReceivingOperations() {
  ...
  const load = async () => {
    setLoading(true);
    try {
      const [warehouses, receivingStaff, shifts] = await Promise.all([
        receivingService.getManagerWarehouses(),
        receivingService.getManagerReceivingStaff(),
      ...
      toast.error(e?.response?.data?.message || 'Không thể tải dữ liệu vận hành.');
    } finally {
      setLoading(false);
    }
  };
  ...
  const upcoming = useMemo(
    () =>
      setup.shifts.filter((x) => x.shiftDate.slice(0, 10) >= today() || x.status !== 'Completed'),
    [setup.shifts],
  );
  const stats = {
  ...
  const createShifts = async () => {
    ...
    try {
      await receivingService.generateStandardShifts(warehouseId, date);
      toast.success('Đã tạo ca sáng và ca chiều.');
      await load();
    } catch (e: any) {
      toast.error(e?.response?.data?.message || 'Không thể tạo ca làm việc.');
    } finally {
      setCreatingShifts(false);
    }
  };
  const parsedHolidayDates = () =>
    holidayText
      .split(/[\s,;]+/)
      .map((x) => x.trim())
      .filter(Boolean);
  ...
  const createYearShifts = async () => {
    ...
    try {
      ...
      toast.success(
        `Đã tạo ${result.createdShifts} ca trên ${result.workingDays} ngày; bỏ qua ${result.skippedExisting} ca đã có.`,
      );
      setConfirmYearOpen(false);
      await load();
    } catch (e: any) {
      toast.error(e?.response?.data?.message || 'Không thể tạo lịch làm việc cả năm.');
    } finally {
  ...
  const openTeam = (shift: ManagerShiftOverview) => {
    setTeamShift(shift);
    setTeamName(`Team ${shift.shiftName} ${new Date(shift.shiftDate).toLocaleDateString('vi-VN')}`);
    setStaffIds([]);
  };
  ...
  const createTeam = async () => {
    ...
    setSavingTeam(true);
    try {
      await receivingService.createTeam(teamShift.id, teamName.trim(), staffIds);
      toast.success('Đã tạo receiving team.');
      setTeamShift(null);
      await load();
    } catch (e: any) {
  ...
  const plan = async (shift: ManagerShiftOverview) => {
    ...
    try {
      ...
      toast.error(e?.response?.data?.message || 'Không thể lập tuyến tự động.');
    } finally {
      ...
      <div className="ops-page manager-ops">
        ...
        <div className="ops-stats">
          <div className="ops-stat-card">
            <span className="ops-stat-label">Ca chờ thực hiện</span>
            <strong className="ops-stat-value">{stats.scheduled}</strong>
          </div>
          <div className="ops-stat-card">
            <span className="ops-stat-label">Team đã lập</span>
          ...
          <div className="ops-stat-card">
            <span className="ops-stat-label">Đơn đã phân công</span>
        ...
        <section className="ops-panel manager-shift-create">
          <div className="ops-section-head">
            <div>
              <span className="ops-panel-label">Bước 1</span>
              <h2>Tạo ca chuẩn</h2>
            </div>
            <span>Tự tạo 08:00–11:00 và 13:00–17:00</span>
          </div>
          {setup.warehouses.length === 0 ? (
            <div className="ops-empty">
              <Warehouse size={34} />
              <h4>Chưa có kho hoạt động</h4>
              <p>Hãy tạo Warehouse trước khi lập ca.</p>
            </div>
          ) : (
            <div className="manager-create-grid">
              ...
              >
                <Plus size={16} />
                {creatingShifts ? 'Đang tạo...' : 'Tạo 2 ca làm'}
              </button>
            </div>
          )}
          <div className="manager-year-divider">
            <span>HOẶC TẠO LỊCH CẢ NĂM</span>
          </div>
          <div className="manager-year-grid">
            <div className="ops-field">
              <label>Năm làm việc</label>
              <input
                type="number"
                min="2020"
                max="2100"
            ...
            <div className="ops-field">
              ...
              <textarea
                ...
                placeholder={`${year}-02-16, ${year}-02-17, ${year}-04-26`}
              />
              <small>
                Nhập ngày Tết, Giỗ Tổ… cách nhau bằng dấu phẩy. Hệ thống tự bỏ cuối tuần và 01/01,
                30/04, 01/05, 02/09.
              </small>
            </div>
            <button
              ...
              onClick={requestYearShifts}
            >
              <CalendarDays size={16} />
              {creatingYear ? 'Đang tạo lịch...' : `Tạo lịch năm ${year}`}
            </button>
          </div>
        </section>
          ...
          <div className="manager-shift-grid">
              ...
              <article className="manager-shift-card" key={shift.id}>
                ...
                {shift.team ? (
                  <div className="manager-team-summary">
                    <div>
                      <Users size={18} />
                      <strong>{shift.team.teamName}</strong>
                    </div>
                    {shift.team.members.map((x) => (
                      <span key={x.id}>
                  ...
                  <button
                    ...
                    onClick={() => openTeam(shift)}
                  >
                    <UserPlus size={16} /> Tạo team cho ca
                  </button>
                )}
                <div className="manager-batch-summary">
                  <span>
                    <PackageCheck size={15} />
                    {shift.intakeBatchCode || 'Chưa có Intake Batch'}
                  </span>
                  <strong>{shift.assignedRequests} đơn</strong>
                </div>
                ...
                {shift.team && shift.status === 'Scheduled' && (
                  <button
                    ...
                    disabled={planningId === shift.id}
                    onClick={() => plan(shift)}
                  >
                    <Truck size={16} />
                    {planningId === shift.id ? 'Đang lập tuyến...' : 'Tự động lấy đơn phù hợp'}
                  </button>
                )}
              </article>
            ))}
            {!loading && !upcoming.length && (
              <div className="ops-empty">
                ...
                <h4>Chưa có ca làm</h4>
                <p>Tạo ca chuẩn ở bước 1.</p>
              </div>
            )}
          </div>
        </section>
        ...
        <div>
          <div className="ops-section-head">
            <div>
              <span className="ops-panel-label">Bước 3</span>
              <h2>Điều phối từng đơn</h2>
            </div>
            <span>Chọn team cùng kho cho từng Donation Request</span>
          </div>
        ...
        {teamShift && (
          <div className="manager-modal-backdrop">
            <section className="ops-panel manager-team-modal">
              <div className="ops-section-head">
                <div>
                  <span className="ops-panel-label">{teamShift.shiftName}</span>
                  <h2>Tạo Receiving Team</h2>
                </div>
              ...
              <div className="manager-staff-list">
                {setup.receivingStaff.map((staff) => (
                  <button
                    type="button"
                    className={staffIds.includes(staff.id) ? 'selected' : ''}
        ...
        {confirmYearOpen && (
          <div
            ...
            role="presentation"
            onMouseDown={(e) => {
              if (e.target === e.currentTarget && !creatingYear) setConfirmYearOpen(false);
            }}
          >
            <section
              className="ops-panel manager-year-modal"
              role="dialog"
              aria-modal="true"
              aria-labelledby="confirm-year-title"
            >
              <div className="manager-year-modal-icon">
              ...
              <div className="manager-year-modal-title">
                ...
                <p>
                  Hệ thống sẽ thêm ca cho tất cả ngày làm việc hợp lệ và tự bỏ qua ca đã tồn tại.
                </p>
              </div>
              <div className="manager-year-confirm-grid">
                <div>
                  <span>Kho áp dụng</span>
                  <strong>{setup.warehouses.find((x) => x.id === warehouseId)?.name}</strong>
                </div>
                <div>
                  <span>Ca mỗi ngày</span>
                  <strong>2 ca · Sáng và chiều</strong>
                </div>
                <div>
                  <span>Cuối tuần</span>
                  <strong>Không tạo Thứ 7, Chủ nhật</strong>
                </div>
                <div>
                  <span>Ngày lễ bổ sung</span>
                  <strong>{parsedHolidayDates().length} ngày</strong>
                </div>
              </div>
              <div className="manager-year-holidays">
                <strong>Ngày hệ thống tự loại</strong>
                <span>01/01 · 30/04 · 01/05 · 02/09</span>
                {parsedHolidayDates().length > 0 && (
                  <small>
                    Thêm:{' '}
                    {parsedHolidayDates()
                      .map((x) => new Date(`${x}T00:00:00`).toLocaleDateString('vi-VN'))
                      .join(' · ')}
                  </small>
                )}
              </div>
              <div className="manager-year-modal-actions">
                <button
                  ...
                  onClick={() => setConfirmYearOpen(false)}
                >
                  Hủy bỏ
                </button>
                <button
                  className="ops-btn ops-btn-primary"
                  disabled={creatingYear}
                  onClick={createYearShifts}
                >
                  <CalendarDays size={16} />
                  {creatingYear ? 'Đang tạo lịch...' : 'Xác nhận tạo lịch'}
                </button>
              </div>
            </section>
          </div>
