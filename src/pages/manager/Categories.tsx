...
const blank = { id:'', code:'', name:'', type:'FabricType', parentId:'', sortOrder:1, description:'', minimumMatchCount:null as number|null };
...
export default function Categories(){
  ...
  const [gradeThresholds,setGradeThresholds]=useState<Record<string,number>>({GRADE_B:2,GRADE_C:1});
  ...
  const maximumCategoryOrder=form.id&&originalCategory?.type===form.type?categoryTypeCount:categoryTypeCount+1;
  const savedGradeThresholds=useMemo(()=>{
    const grades=items.filter(x=>x.type==='ConditionGrade');
    return {GRADE_B:grades.find(x=>x.code==='GRADE_B')?.minimumMatchCount??2,GRADE_C:grades.find(x=>x.code==='GRADE_C')?.minimumMatchCount??1};
  },[items]);
  const gradeRulesChanged=gradeThresholds.GRADE_B!==savedGradeThresholds.GRADE_B||gradeThresholds.GRADE_C!==savedGradeThresholds.GRADE_C;
  const edit=(item:Category)=>{setForm({...blank,...item,parentId:item.parentId||''});setOpen(true);};
  const create=()=>{setForm({...blank,type:filter,sortOrder:visible.length+1});setOpen(true);};
  const save=async()=>{
    ...
    if(form.sortOrder<1||form.sortOrder>maximumCategoryOrder){toast.error(`Thứ tự phải từ 1 đến ${maximumCategoryOrder}.`);return;}
    setSaving(true);
  ...
  useEffect(()=>{const next={...gradeThresholds};items.filter(x=>x.type==='ConditionGrade').forEach(item=>{if(item.code==='GRADE_B'||item.code==='GRADE_C')next[item.code]=item.minimumMatchCount??(item.code==='GRADE_B'?2:1);});setGradeThresholds(next);},[items]);
  const saveGradeRules=async()=>{
    const gradeItems=items.filter(x=>x.type==='ConditionGrade'&&(x.code==='GRADE_B'||x.code==='GRADE_C'));
    if(gradeItems.length!==2)return toast.error('Cần cấu hình đầy đủ Nhãn B và Nhãn C.');
    if(gradeItems.some(item=>(gradeThresholds[item.code]||0)<1))return toast.error('Ngưỡng phải từ 1 trở lên.');
    setSavingRules(true);
    try{await Promise.all(gradeItems.map(item=>apiClient.put(`/categories/${item.id}`,{...item,minimumMatchCount:gradeThresholds[item.code]})));toast.success('Đã lưu quy tắc tổng hợp nhãn.');await load();}
    catch(e:any){toast.error(e?.response?.data?.message||'Không thể lưu quy tắc tổng hợp nhãn.');}
  ...
  return <AdminLayout><div className="ops-page">
    ...
    <section className="ops-panel"><div className="ops-section-head"><h2>{types.find(x=>x[0]===filter)?.[1]}</h2><span>{visible.length} giá trị đang sử dụng</span></div>
      {filter==='ConditionGrade'&&<div className="grade-rule-config"><div className="grade-rule-intro"><strong>Quy tắc tổng hợp kết quả</strong><span>Hệ thống xét C trước, sau đó B; nếu không đạt ngưỡng thì kết quả là A.</span>{gradeRulesChanged&&<em>Đang có thay đổi chưa lưu</em>}</div><div className="grade-rule-fields"><label><span>Nhãn C</span><small>Số câu trả lời C tối thiểu</small><input type="number" min="1" value={gradeThresholds.GRADE_C} onChange={e=>setGradeThresholds({...gradeThresholds,GRADE_C:Number(e.target.value)})}/></label><label><span>Nhãn B</span><small>Số câu trả lời B tối thiểu</small><input type="number" min="1" value={gradeThresholds.GRADE_B} onChange={e=>setGradeThresholds({...gradeThresholds,GRADE_B:Number(e.target.value)})}/></label><div className="grade-rule-example"><b>Quy tắc hiện tại</b><span>Chỉ cần {savedGradeThresholds.GRADE_C} câu C → Nhãn C</span><span>Không đạt C nhưng có {savedGradeThresholds.GRADE_B} câu B → Nhãn B</span><span>Còn lại → Nhãn A</span></div></div><button className="ops-btn ops-btn-primary" disabled={savingRules||!gradeRulesChanged} onClick={saveGradeRules}><Save size={16}/>{savingRules?'Đang lưu...':gradeRulesChanged?'Lưu quy tắc':'Đã cập nhật'}</button></div>}
      <div className="ops-item-list">{visible.map(item=><div className="ops-item-row category-item-row" key={item.id}>
    ...
    {open&&<div className="rcv-modal-overlay"><section className="ops-panel rcv-modal">
      ...
      <div className="ops-field"><label>Loại cấu hình</label><select value={form.type} onChange={e=>{const type=e.target.value;setForm(p=>({...p,type,parentId:'',sortOrder:items.filter(x=>x.type===type).length+1}))}}>{types.map(([v,l])=><option value={v} key={v}>{l}</option>)}</select></div>
      ...
      <div className="ops-field"><label>Thứ tự hiển thị *</label><input type="number" min="1" max={maximumCategoryOrder} value={form.sortOrder} onChange={e=>setForm(p=>({...p,sortOrder:Number(e.target.value)}))}/><small>{form.id&&originalCategory?.type===form.type?`Từ 1 đến ${maximumCategoryOrder}. Chọn vị trí đã có để đổi chỗ.`:`Từ 1 đến ${maximumCategoryOrder}. Các giá trị phía sau sẽ tự dịch xuống.`}</small></div>
