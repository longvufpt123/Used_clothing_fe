...
import {
  Activity, Boxes, ClipboardList, PackageCheck, RefreshCw,
...
const EmptyChart = () => (
  <div className="manager-chart-empty">Chưa có dữ liệu phát sinh</div>
);
...
export default function ManagerDashboard() {
  ...
  const load = useCallback(async () => {
    try {
      ...
      setData(await apiClient.get<DashboardData>(`/manager-dashboard${suffix}`) as unknown as DashboardData);
    } catch {
  ...
  useEffect(() => {
    receivingService.getManagerSetup()
      .then(setup => setWarehouses(setup.warehouses))
      .catch(() => setWarehouses([]));
  }, []);

  const daily = useMemo(() => (data?.lastSevenDays ?? []).map(item => ({
    ...item,
    day: data?.trendGranularity === 'month'
  ...
  const hasDailyData = daily.some(x => x.donationRequests || x.inboundBatches || x.outboundBatches);
    ...
    <AdminLayout role="manager">
      <div className="admin-dashboard manager-dashboard">
        <header className="manager-dashboard-header">
          ...
          <button type="button" className="manager-refresh-btn" onClick={() => void load()} disabled={loading}>
        ...
        <section className="manager-dashboard-filters">
          ...
          <div className="dashboard-filter-field">
            <label>Tháng</label>
            <select value={month} disabled={!year} onChange={event => { setMonth(event.target.value); setDate(''); }}>
          ...
          {(warehouseId || year || month || date) && (
            <button type="button" className="dashboard-clear-filters" onClick={() => {
              setWarehouseId(''); setYear(''); setMonth(''); setDate('');
            }}>Xóa bộ lọc</button>
          )}
        </section>
        ...
        <section className="manager-summary-grid" aria-busy={loading}>
          <article className="manager-summary-card">
            <div className="summary-icon green"><ClipboardList size={21} /></div>
            <div><span>Donation Request</span><strong>{data?.totalDonationRequests ?? '—'}</strong><small>Tổng đơn quyên góp</small></div>
          </article>
          <article className="manager-summary-card">
            <div className="summary-icon blue"><Boxes size={21} /></div>
            <div><span>Intake Batch</span><strong>{data?.totalIntakeBatches ?? '—'}</strong><small>Tổng lô tiếp nhận</small></div>
          </article>
          <article className="manager-summary-card">
            <div className="summary-icon purple"><PackageCheck size={21} /></div>
            <div><span>Classified Batch</span><strong>{data?.totalClassifiedBatches ?? '—'}</strong><small>Tổng lô đã gom nhóm</small></div>
          </article>
          <article className="manager-summary-card warehouse-summary">
            <div className="summary-icon orange"><Warehouse size={21} /></div>
            <div>
              <span>Luân chuyển kho</span>
              <strong>{(data?.warehouseFlow.inboundTransactions ?? 0) + (data?.warehouseFlow.outboundTransactions ?? 0)}</strong>
        ...
        <section className="manager-chart-grid">
          <ChartCard title="Trạng thái Donation Request" subtitle="Số lượng đơn theo tiến độ tiếp nhận" data={data?.donationRequestPipeline} />
          <ChartCard title="Tiến độ Intake Batch" subtitle="Các lô đang thu gom và đã hoàn tất" data={data?.intakeBatchPipeline} />
          <ChartCard title="Tiến độ phân loại" subtitle="Intake Batch trong quy trình phân loại" data={data?.classificationPipeline} />
          <ChartCard title="Luồng batch trong kho" subtitle="Gom nhóm, chờ nhập, đã nhập và đã xuất" data={data?.warehouseBatchPipeline} />
        </section>

        <section className="manager-flow-row">
          <article className="manager-wide-chart">
            <div className="manager-chart-heading">
              <div><h2>Hoạt động 7 ngày gần nhất</h2><p>Đơn quyên góp và số batch nhập/xuất theo ngày</p></div>
              <Activity size={20} />
            </div>
            <div className="manager-wide-chart-body">
              {!hasDailyData ? <EmptyChart /> : (
                <ResponsiveContainer width="100%" height={265}>
                  <AreaChart data={daily} margin={{ top: 12, right: 12, left: -18, bottom: 0 }}>
                    ...
                    <Area type="monotone" name="Batch xuất kho" dataKey="outboundBatches" stroke="#ffb44a" strokeWidth={2} fill="transparent" />
                  </AreaChart>
                </ResponsiveContainer>
              )}
            </div>
          ...
          <article className="manager-warehouse-flow">
            <div className="manager-chart-heading">
              <div><h2>Xuất / nhập kho</h2><p>Số giao dịch và khối lượng thực tế</p></div>
              <Warehouse size={20} />
            </div>
            <div className="warehouse-flow-stat inbound">
              <TrendingUp size={22} />
              <div><span>Nhập kho</span><strong>{data?.warehouseFlow.inboundTransactions ?? 0} batch</strong></div>
              <b>{data?.warehouseFlow.inboundWeightKg ?? 0} kg</b>
            </div>
            <div className="warehouse-flow-stat outbound">
              <TrendingDown size={22} />
              <div><span>Xuất kho</span><strong>{data?.warehouseFlow.outboundTransactions ?? 0} batch</strong></div>
...
function ChartCard({ title, subtitle, data = [] }: { title: string; subtitle: string; data?: Metric[] }) {
  const hasData = data.some(x => x.count > 0);
  return (
    <article className="manager-chart-card">
      <div className="manager-chart-heading"><div><h2>{title}</h2><p>{subtitle}</p></div></div>
