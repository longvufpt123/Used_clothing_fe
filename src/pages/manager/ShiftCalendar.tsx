...
export default function ShiftCalendar(){
  ...
  const createDay=async()=>{if(!warehouseId)return toast.warning('Chọn kho.');setCreating(true);try{const shiftDefinitions=[{name:'Ca sáng',startTime:`${scheduleTimes.morningStartTime}:00`,endTime:`${scheduleTimes.morningEndTime}:00`},{name:'Ca chiều',startTime:`${scheduleTimes.afternoonStartTime}:00`,endTime:`${scheduleTimes.afternoonEndTime}:00`}];await receivingService.generateShifts({warehouseId,startDate:selected,periodUnit:0,periodValue:1,workingDays:[0,1,2,3,4,5,6],excludedDates:[],shiftDefinitions});toast.success('Đã tạo ca sáng và ca chiều.');await load();}catch(e:any){toast.error(e?.response?.data?.message||'Không thể tạo ca.');}finally{setCreating(false);}};
  ...
  const createYear=async()=>{setCreatingYear(true);try{const shiftDefinitions=[{name:'Ca sáng',startTime:`${scheduleTimes.morningStartTime}:00`,endTime:`${scheduleTimes.morningEndTime}:00`},{name:'Ca chiều',startTime:`${scheduleTimes.afternoonStartTime}:00`,endTime:`${scheduleTimes.afternoonEndTime}:00`}];const result=await receivingService.generateShifts({warehouseId,startDate:`${year}-01-01`,periodUnit:4,periodValue:1,workingDays,excludedDates:holidayDates(),shiftDefinitions});toast.success(`Đã tạo ${result.createdShifts} ca trên ${result.workingDays} ngày làm việc; bỏ qua ${result.skippedExisting} ca đã có.`);setYearOpen(false);setMonth(new Date(year,0,1));await load();}catch(e:any){toast.error(e?.response?.data?.message||'Không thể tạo lịch năm.');}finally{setCreatingYear(false);}};
  ...
  const createMonth=async()=>{setCreatingMonth(true);try{const startDate=`${monthYear}-${String(monthIndex).padStart(2,'0')}-01`;const shiftDefinitions=[{name:'Ca sáng',startTime:`${scheduleTimes.morningStartTime}:00`,endTime:`${scheduleTimes.morningEndTime}:00`},{name:'Ca chiều',startTime:`${scheduleTimes.afternoonStartTime}:00`,endTime:`${scheduleTimes.afternoonEndTime}:00`}];const result=await receivingService.generateShifts({warehouseId,startDate,periodUnit:2,periodValue:1,workingDays,excludedDates:monthHolidayDates(),shiftDefinitions});toast.success(`Đã tạo ${result.createdShifts} ca trên ${result.workingDays} ngày làm việc; bỏ qua ${result.skippedExisting} ca đã có.`);setMonthOpen(false);setMonth(new Date(monthYear,monthIndex-1,1));await load();}catch(e:any){toast.error(e?.response?.data?.message||'Không thể tạo lịch tháng.');}finally{setCreatingMonth(false);}};
