...
import type {
  ManagerReceivingSetup,
  ManagerShiftOverview,
  ManagerTeamOverview,
...
export default function DispatchOperations() {
  ...
  const load = async (detailId?: string) => {
    ...
    try {
      ...
      const selectedWarehouse = warehouseFilter || warehouses[0]?.id || '';
      const [receivingStaff, shifts] = selectedWarehouse
        ? await Promise.all([
  ...
  const years = useMemo(
    () => Array.from(new Set(shifts.map((x) => x.shiftDate.slice(0, 4)))).sort(),
    [shifts],
  );
    ...
    () =>
      shifts.filter(
        (x) =>
          ...
          (!dateFilter || x.shiftDate.slice(0, 10) === dateFilter),
      ),
    [shifts, warehouseFilter, yearFilter, dateFilter],
  );
      ...
      Array.from(
        filtered
          .reduce((groups, shift) => {
            ...
            return groups;
          }, new Map<string, { key: string; date: string; warehouseName: string; shifts: ManagerShiftOverview[] }>())
          .values(),
      )
        .map((day) => ({
          ...day,
          shifts: day.shifts.sort((a, b) => a.startTime.localeCompare(b.startTime)),
        }))
  ...
  const openCreateTeam = (
    shift: ManagerShiftOverview,
    type: 'ReceivingPickup' | 'ReceivingWarehouse' = 'ReceivingPickup',
  ) => {
    setTeamType(type);
    setTeamShift(shift);
    setTeamName(
  ...
  const assignedTeam = (shift: ManagerShiftOverview, staffId: string, exceptTeamId?: string) =>
    shift.teams.find(
      (team) => team.id !== exceptTeamId && team.members.some((member) => member.id === staffId),
    );
  ...
  const createTeam = async () => {
    ...
    try {
      ...
      toast.error(e?.response?.data?.message || 'Không thể tạo team.');
    } finally {
      setSavingTeam(false);
    }
  };
  ...
  const startEditTeam = (team: ManagerTeamOverview) => {
    ...
    setEditTeamStaffIds(team.members.map((x) => x.id));
    setStaffSearch('');
    setCollapsedTeamIds((current) => {
      const next = new Set(current);
      next.delete(team.id);
  ...
  const saveTeam = async () => {
    ...
    try {
      ...
      toast.error(e?.response?.data?.message || 'Không thể cập nhật team.');
    } finally {
  ...
  const deleteTeam = async (team: ManagerTeamOverview) => {
    ...
    try {
      await receivingService.deleteTeam(team.id);
      toast.success('Đã xóa team.');
      await load(detailShift.id);
    } catch (e: any) {
      toast.error(e?.response?.data?.message || 'Không thể xóa team.');
    } finally {
  ...
  const moveRequest = async (requestId: string, teamId: string) => {
    ...
    try {
      ...
      toast.success('Đã chuyển đơn sang team mới.');
      await load(detailShift.id);
    } catch (e: any) {
      toast.error(e?.response?.data?.message || 'Không thể chuyển đơn.');
    }
  };

  const transferTeams = detailShift
    ? setup.shifts
        .filter(
          (shift) =>
            shift.warehouseId === detailShift.warehouseId &&
            shift.shiftDate.slice(0, 10) === detailShift.shiftDate.slice(0, 10) &&
  ...
  const saveShift = async () => {
    ...
    try {
      ...
      toast.error(e?.response?.data?.message || 'Không thể cập nhật ca.');
    } finally {
  ...
  const removeShift = async () => {
    ...
    try {
      ...
      toast.error(e?.response?.data?.message || 'Không thể xóa ca.');
    } finally {
      setDeletingShift(false);
    }
  };

  const staffAt = (warehouseId: string, query: string) =>
    setup.receivingStaff
      .filter((x) => x.warehouseId === warehouseId)
      .filter((x) => {
        const q = normalize(query);
        return !q || normalize(x.fullName).includes(q) || x.phoneNumber.includes(q);
      });
    ...
    <AdminLayout>
      <div className="ops-page manager-ops">
        ...
        <div className="ops-stats">
          ...
          <div className="ops-stat-card">
            <span className="ops-stat-label">Ca đang chạy</span>
            <strong className="ops-stat-value">{stats.active}</strong>
          </div>
          <div className="ops-stat-card">
            <span className="ops-stat-label">Đơn đã assign</span>
            <strong className="ops-stat-value">{stats.assigned}</strong>
          </div>
        </div>
        ...
        <section>
          <div className="ops-section-head">
            ...
            <span>
              {workDays.length} ngày · {filtered.length} ca
            </span>
          </div>
          <div className="manager-shift-toolbar">
            <div className="manager-date-filter">
              ...
              <input
                ...
                onChange={(e) => {
                  ...
                  if (e.target.value) setYearFilter(e.target.value.slice(0, 4));
                }}
              />
            ...
            <span>
              {workDays.length ? (page - 1) * pageSize + 1 : 0}–
              {Math.min(page * pageSize, workDays.length)} / {workDays.length} ngày
            </span>
          </div>
          <div className="manager-workday-list">
            {pagedDays.map((day) => {
              ...
              return (
                <article className="manager-workday-card" key={day.key}>
                  <div className="manager-workday-head">
                    <div>
                      ...
                      <span>
                        ...
                        <small>{day.warehouseName}</small>
                      </span>
                    </div>
                    <b>
                      {teamCount} team · {requestCount} đơn
                    </b>
                  </div>
                  <div className="manager-workday-shifts">
                    {day.shifts.map((shift) => (
                      <section
                        className="manager-day-shift"
                      ...
                      >
                          ...
                          {(shift.status === 'Scheduled' || shift.status === 'InProgress') && (
                            <div className="manager-shift-team-actions">
                              <button
                                ...
                                title={`Thêm pickup team vào ${shift.shiftName}`}
                              >
                                <UserPlus size={14} /> Pickup
                              </button>
                              {shift.pendingDropOffRequests > 0 &&
                                !shift.teams.some((x) => x.teamType === 'ReceivingWarehouse') && (
                                  <button
                                    className="warehouse-team-btn"
                                    onClick={(e) => {
                                      e.stopPropagation();
                                      openCreateTeam(shift, 'ReceivingWarehouse');
                                    }}
                                    title={`Tạo team trực kho cho ${shift.shiftName}`}
                                  >
                                    <Warehouse size={14} /> Trực kho
                                  </button>
                                )}
                            </div>
                          )}
                        ...
                        {shift.pendingDropOffRequests > 0 && (
                          <small className="manager-dropoff-demand">
                            {shift.pendingDropOffRequests} đơn dự kiến mang đến kho trong ngày
                          </small>
                        )}
                      </section>
                    ))}
          ...
          {totalPages > 1 && (
            <nav className="manager-shift-pagination">
              ...
              {pages(page, totalPages).map((x) =>
                typeof x === 'number' ? (
                  <button className={x === page ? 'active' : ''} onClick={() => setPage(x)} key={x}>
                    {x}
                  </button>
                ) : (
                  <span className="manager-page-gap" key={x}>
                    …
                  </span>
                ),
              )}
        ...
        {detailShift && (
          <div
            ...
            onMouseDown={(e) => {
              if (e.target === e.currentTarget && !savingShift) setDetailShift(null);
            }}
          >
            <section className="ops-panel teams-shift-detail manager-multi-detail">
              <div className="teams-detail-head">
                <div>
                  <span>CHI TIẾT CA · {detailShift.teams.length} TEAM</span>
                  <h2>{detailShift.shiftName}</h2>
                  <p>
                ...
                <button onClick={() => setDetailShift(null)}>×</button>
              </div>
              {editingShift ? (
                ...
                <>
                  <div className="teams-detail-summary">
                    ...
                    <div>
                      <span>Tổng tải</span>
                      <strong>
                        {detailShift.teams.length} team · {detailShift.assignedRequests} đơn
                      </strong>
                    </div>
                  </div>
                  <div className="manager-team-command">
                    <button
                    ...
                    {detailShift.pendingDropOffRequests > 0 &&
                      !detailShift.teams.some((x) => x.teamType === 'ReceivingWarehouse') && (
                        <button
                          className="ops-btn ops-btn-secondary"
                  ...
                  <div className="manager-multi-team-list">
                    {detailShift.teams.map((team) => {
                      const received = team.requests.filter(
                        (request) => request.status === 'Received',
                      ).length;
                      const startedBy = team.members.find(
                        (member) => member.id === team.startedByStaffId,
                      )?.fullName;
                      const completedBy = team.members.find(
                        (member) => member.id === team.completedByStaffId,
                      )?.fullName;
                      return (
                        <section className="manager-multi-team" key={team.id}>
                          <div className="manager-multi-team-head">
                            <div>
                              {team.teamType === 'ReceivingWarehouse' ? (
                                <Warehouse size={18} />
                              ) : (
                              ...
                              <div>
                                ...
                                <span>
                                  {team.teamType === 'ReceivingWarehouse'
                                    ? 'Team trực kho'
                                    : 'Team đi thu gom'}{' '}
                                  · {team.members.length} thành viên · đã thu {received}/
                                  {team.requests.length} đơn
                                </span>
                              </div>
                            </div>
                            <div>
                              ...
                              <button
                                ...
                                disabled={team.status !== 'Scheduled'}
                              >
                                <Pencil size={14} /> Sửa
                              </button>
                              <button
                            ...
                            (editingTeamId === team.id ? (
                              <div className="manager-edit-team">
                                ...
                                <div className="manager-staff-list">
                                  {staffAt(detailShift.warehouseId, staffSearch).map((staff) => {
                                    const occupied = assignedTeam(detailShift, staff.id, team.id);
                                    return (
                                      <button
                                        className={`${editTeamStaffIds.includes(staff.id) ? 'selected' : ''}${occupied ? ' assigned' : ''}`}
                                        onClick={() => toggleEditStaff(staff.id)}
                                        disabled={!!occupied}
                                        key={staff.id}
                                      >
                                        <span>
                                          ...
                                          {occupied && (
                                            <small className="manager-assigned-note">
                                              Đã thuộc {occupied.teamName}
                                            </small>
                                          )}
                                        </span>
                                        {editTeamStaffIds.includes(staff.id) ? (
                                          <CheckCircle2 size={18} />
                                        ) : (
                                          <span className="manager-check" />
                                        )}
                                      </button>
                                    );
                                  })}
                                ...
                                <div className="manager-inline-actions">
                                  <button
                                    className="ops-btn ops-btn-secondary"
                                    onClick={() => setEditingTeamId(undefined)}
                                  >
                                    Hủy
                                  </button>
                                  <button
                              ...
                              <>
                                <div className="manager-team-audit">
                                  ...
                                  <div>
                                    ...
                                    <strong>
                                      {formatAuditTime(team.startedAt)}
                                      {startedBy ? ` · ${startedBy}` : ''}
                                    </strong>
                                  </div>
                                  <div>
                                    ...
                                    <strong>
                                      {formatAuditTime(team.completedAt)}
                                      {completedBy ? ` · ${completedBy}` : ''}
                                    </strong>
                                  </div>
                                  <div>
                                    <span>Kết quả thu gom</span>
                                    <strong>
                                      {received}/{team.requests.length} đơn ·{' '}
                                      {team.intakeBatchWeight || 0} kg
                                    </strong>
                                  </div>
                                </div>
                                <div className="manager-team-members">
                                ...
                                {isPickupTeam(team) && (
                                  <button
                                    ...
                                    disabled={!team.requests.length}
                                  >
                                    <MapPinned size={16} />
                                    Xem tuyến đường trên bản đồ
                                  </button>
                                )}
                                <div className="manager-team-requests">
                                  {team.requests.map((request) => (
                                    <div key={request.id}>
                                      <span>
                                        <strong>
                                          {request.routeOrder}. {request.code}
                                        </strong>
                                        <small>
                                  ...
                                  {!team.requests.length && (
                                    <div className="teams-detail-empty">Team chưa có đơn.</div>
                                  )}
                                </div>
                              </>
                            ))}
              ...
              {deleteConfirm && (
                <div className="teams-delete-confirm">
                  ...
                  <span>Chỉ xóa được khi ca không còn team, batch hoặc đơn.</span>
                  <div>
                    <button
                      className="ops-btn ops-btn-secondary"
                    ...
                    >
                      {deletingShift ? 'Đang xóa...' : 'Xác nhận xóa'}
                    </button>
                  </div>
                </div>
              )}
                ...
                {editingShift ? (
                  ...
                  <>
                    <button
                      ...
                      disabled={detailShift.status !== 'Scheduled' || !!detailShift.teams.length}
                    >
        ...
        {routeTeamId &&
          ...
          (() => {
            ...
            return (
              <div
                className="manager-modal-backdrop manager-route-modal-backdrop"
                onMouseDown={(e) => {
                  if (e.target === e.currentTarget) setRouteTeamId(undefined);
                }}
              >
                <section className="ops-panel manager-route-modal">
                  <div className="teams-detail-head">
                    <div>
                      <span>TUYẾN ĐƯỜNG THU GOM · {routeTeam.requests.length} ĐƠN</span>
                      <h2>{routeTeam.teamName}</h2>
                      <p>
                        {routeTeam.intakeBatchCode || 'Intake Batch chưa có mã'} ·{' '}
                        {detailShift.warehouseName}
                      </p>
                    </div>
                    <button onClick={() => setRouteTeamId(undefined)}>×</button>
                  </div>
                  <RouteMap
                    autoBuild
                    batch={{
                      warehouseAddress:
                        setup.warehouses.find((x) => x.id === detailShift.warehouseId)?.address ||
                        detailShift.warehouseName,
                      requests: [...routeTeam.requests]
                  ...
                  />
                  <div className="manager-route-modal-actions">
                    <button
                      className="ops-btn ops-btn-primary"
                      onClick={() => setRouteTeamId(undefined)}
                    >
                      Đóng bản đồ
                    </button>
                  </div>
        ...
        {teamShift && (
          <div
            className="manager-modal-backdrop"
            onMouseDown={(e) => {
              if (e.target === e.currentTarget && !savingTeam) setTeamShift(null);
            }}
          >
            <section className="ops-panel manager-team-modal">
              <div className="ops-section-head">
                <div>
                  <span className="ops-panel-label">
                    {teamShift.shiftName} · {teamShift.warehouseName}
                  </span>
                  <h2>
                    {teamType === 'ReceivingWarehouse'
                      ? 'Tạo team Receiving trực kho'
                      : 'Thêm team đi thu gom'}
                  </h2>
                  <p>
                    {teamType === 'ReceivingWarehouse'
                      ? 'Team tiếp nhận donor mang hàng đến kho, không tham gia tuyến thu gom.'
                      : 'Team đến lấy Donation Request tại địa chỉ donor.'}
                  </p>
                </div>
                <button className="manager-close" onClick={() => setTeamShift(null)}>
                  ×
                </button>
              </div>
              <div className="ops-field">
              ...
              <div className="manager-staff-list">
                {staffAt(teamShift.warehouseId, staffSearch).map((staff) => {
                  const occupied = assignedTeam(teamShift, staff.id);
                  return (
                    <button
                      className={`${staffIds.includes(staff.id) ? 'selected' : ''}${occupied ? ' assigned' : ''}`}
                      onClick={() => toggleCreateStaff(staff.id)}
                      disabled={!!occupied}
                      key={staff.id}
                    >
                      <span>
                        ...
                        {occupied && (
                          <small className="manager-assigned-note">
                            Đã thuộc {occupied.teamName} trong ca này
                          </small>
                        )}
                      </span>
