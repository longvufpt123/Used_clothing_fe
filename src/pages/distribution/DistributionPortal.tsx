...
export default function DistributionPortal({ mode }: { mode: Mode }) {
  ...
  useEffect(() => {
    ...
    const close = (event: KeyboardEvent) =>
      event.key === "Escape" && setActiveBatch(null);
  ...
  const catalogPageCount = Math.max(
    1,
    Math.ceil(shown.length / catalogPageSize),
  );
  ...
  const filteredRequests = useMemo(() => {
    ...
    return requests.filter((request) =>
      ...
      (!query || `${request.code} ${request.organizationName} ${request.recipientName} ${request.recipientPhone} ${request.toAddress}`.toLowerCase().includes(query)),
    );
  ...
  const pagedRequests = filteredRequests.slice((requestPage - 1) * requestPageSize, requestPage * requestPageSize);
  useEffect(() => {
    setCatalogPage(1);
  }, [warehouseId, search]);
  useEffect(() => setRequestPage(1), [requestSearch, requestStatus, requestWarehouse, requestDate, organizationView]);
  useEffect(() => {
  ...
  const create = async () => {
    if (!form.recipientName.trim() || !form.recipientPhone.trim() || !form.toAddress.trim() || !form.notes.trim())
    ...
    try {
      ...
      if (editingRequestId)
        await distributionService.update(editingRequestId, payload);
  ...
  const startEdit = (request: DistributionRequest) => {
    ...
    setWarehouseId(warehouses.find((warehouse) => warehouse.warehouseName === request.warehouseName)?.id || "");
    setForm({
    ...
    setSelected(
      Object.fromEntries(
        request.items.map((item) => [item.inventoryId, item.requestedQuantity]),
      ),
    );
    setSelectedItemCodes({});
  ...
  const action = async (
    id: string,
    kind: "approve" | "reject" | "issue" | "refresh",
  ) => {
    try {
  ...
  const createGhnShipment = async () => {
    ...
    } finally { setGhnSubmitting(false); }
  };
    ...
    <div className="distribution-page">
        ...
        <>
          ...
          <div className="distribution-catalog">
            {pagedCatalog.map((batch) => (
              <article key={batch.inventoryId}>
                ...
                <strong>
                  {batch.availableQuantity} item · {batch.availableWeight} kg
                  khả dụng
                </strong>
                <button
                ...
                <div className="quantity-picker">
                  <button
                    onClick={() => setQuantity(batch, batch.availableQuantity)}
                  >
                    Chọn tất cả
                  </button>
                  <label>
                    ...
                    <input
                      ...
                      onChange={(e) =>
                        setQuantity(batch, Number(e.target.value))
                      }
                    />
                  </label>
                </div>
          ...
          {shown.length > catalogPageSize && (
            <nav
              ...
              aria-label="Phân trang danh sách batch"
            >
              <span>
                Hiển thị {(catalogPage - 1) * catalogPageSize + 1}–
                {Math.min(catalogPage * catalogPageSize, shown.length)} trong{" "}
                {shown.length} batch
              </span>
              <div>
                <button
          ...
          <section className={`distribution-form${editingRequestId ? " editing" : ""}`}>
            {editingRequestId && <div className="edit-banner"><Pencil /> Đang chỉnh sửa yêu cầu chờ Manager duyệt</div>}
            <h2>Tạo Distribution Request</h2>
            <label className="distribution-field"><span>Tên người/tổ chức nhận <b>*</b></span><input
            ...
            /></label>
            <label className="distribution-field"><span>Số điện thoại <b>*</b></span><input
            ...
            /></label>
            <label className="distribution-field"><span>Địa chỉ nhận hàng <b>*</b></span><input
              placeholder="Nhập địa chỉ nhận hàng"
              value={form.toAddress}
              required
              onChange={(e) => setForm({ ...form, toAddress: e.target.value })}
            /></label>
            <label className="distribution-field full"><span>Mục đích sử dụng / ghi chú <b>*</b></span><textarea
              placeholder="Mô tả mục đích sử dụng"
              value={form.notes}
              required
              onChange={(e) => setForm({ ...form, notes: e.target.value })}
            /></label>
            {editingRequestId && <button className="cancel-edit" onClick={cancelEdit}><X /> Hủy chỉnh sửa</button>}
            <button onClick={create}>
              <Send /> Gửi yêu cầu (
              {Object.values(selected).reduce((a, b) => a + b, 0)} item)
            </button>
          </section>
        </>
      )}
      {(mode !== "organization" || organizationView !== "catalog") && <section className="distribution-requests">
        <h2>
          {mode === "organization"
            ? "Yêu cầu của tổ chức"
            : "Danh sách yêu cầu"}
        ...
        <div className="distribution-request-filters">
          <label><Search size={16}/><input value={requestSearch} onChange={event=>setRequestSearch(event.target.value)} placeholder="Tìm mã yêu cầu, tổ chức, người nhận, SĐT..."/></label>
          <select value={requestStatus} onChange={event=>setRequestStatus(event.target.value)}><option value="">Tất cả trạng thái</option>{requestStatuses.map(status=><option key={status} value={status}>{status}</option>)}</select>
          <select value={requestWarehouse} onChange={event=>setRequestWarehouse(event.target.value)}><option value="">Tất cả kho</option>{requestWarehouses.map(warehouse=><option key={warehouse} value={warehouse}>{warehouse}</option>)}</select>
          <input type="date" value={requestDate} onChange={event=>setRequestDate(event.target.value)}/>
          {(requestSearch||requestStatus||requestWarehouse||requestDate)&&<button type="button" onClick={()=>{setRequestSearch("");setRequestStatus("");setRequestWarehouse("");setRequestDate("")}}><X size={15}/> Xóa lọc</button>}
          <span>{filteredRequests.length}/{requests.length} yêu cầu</span>
        </div>
      ...
      {ghnTarget && (
        <div className="product-modal-backdrop" onMouseDown={() => !ghnSubmitting && setGhnTarget(null)}>
          <section className="ghn-create-modal" role="dialog" aria-modal="true" aria-labelledby="ghn-form-title" onMouseDown={(event) => event.stopPropagation()}>
            <header>
              <div><span>TẠO VẬN ĐƠN GIAO HÀNG</span><h2 id="ghn-form-title">Thông tin vận đơn GHN</h2><p>{ghnTarget.code} · {ghnTarget.warehouseName}</p></div>
              <button type="button" aria-label="Đóng" disabled={ghnSubmitting} onClick={() => setGhnTarget(null)}><X /></button>
            </header>
            <div className="ghn-recipient-summary">
              <div><span>Người nhận</span><strong>{ghnTarget.recipientName}</strong></div>
              <div><span>Số điện thoại</span><strong>{ghnTarget.recipientPhone}</strong></div>
            ...
            <form onSubmit={(event) => { event.preventDefault(); createGhnShipment(); }}>
              ...
              <label className={ghnErrors.paymentTypeId ? "invalid" : ""}>
                <span>Thanh toán phí vận chuyển <b>*</b></span>
              ...
              <label className={ghnErrors.requiredNote ? "invalid" : ""}>
                ...
                <select value={ghnForm.requiredNote} onChange={(event) => setGhnForm((current) => ({ ...current, requiredNote: event.target.value }))}>
                  <option value="KHONGCHOXEMHANG">Không cho xem hàng</option><option value="CHOXEMHANGKHONGTHU">Cho xem hàng, không cho thử</option><option value="CHOTHUHANG">Cho thử hàng</option>
                </select>
              </label>
              <div className="ghn-form-actions">
                <button type="button" disabled={ghnSubmitting} onClick={() => setGhnTarget(null)}>Hủy</button>
                <button type="submit" className="primary" disabled={ghnSubmitting}>{ghnSubmitting ? <RefreshCw className="spinning" /> : <Truck />}{ghnSubmitting ? "Đang tạo vận đơn..." : "Tạo vận đơn GHN"}</button>
              </div>
            </form>
          </section>
        </div>
      ...
      {activeBatch && (
        <div
          ...
          onMouseDown={() => setActiveBatch(null)}
        >
          <section
          ...
          >
            ...
            <div className="product-grid">
              {pagedProducts.map((item) => {
                const isSelected = (
                  selectedItemCodes[activeBatch.inventoryId] || []
            ...
            {activeBatch.items.length > productPageSize && (
              ...
              >
                <span>
                  ...
                  trong {activeBatch.items.length} sản phẩm
                </span>
                <div>
                  <button
            ...
            <footer>
              <div>
                <b>
                  {selected[activeBatch.inventoryId] || 0}/
                  {activeBatch.availableQuantity}
                </b>
