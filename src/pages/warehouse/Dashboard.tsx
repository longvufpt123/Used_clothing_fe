...
export default function WarehouseDashboard(){
  ...
  const shown=batches.filter(batch=>(tab==='inbound'?batch.status==='PendingWarehouseReceipt':tab==='putaway'?batch.status==='WarehouseReceived':batch.status==='Stored')
    &&(!normalized||[batch.batchCode,batch.clothingType,batch.fabricType,batch.conditionGrade,batch.processingDirection,batch.gender,batch.targetUser,batch.size].some(value=>String(value||'').toLowerCase().includes(normalized))));
  const totalPages=Math.max(1,Math.ceil(shown.length/pageSize));const paged=shown.slice((page-1)*pageSize,page*pageSize);
  ...
  return <div className="ops-page">
    ...
    <section><div className="ops-section-head"><h2>Luồng Classified Batch</h2><span>{loading?'Đang đồng bộ...':'Dữ liệu trực tiếp từ hệ thống'}</span></div>
      ...
      <div className="ops-list-toolbar"><label className="ops-list-search"><Search size={17}/><input value={search} onChange={e=>setSearch(e.target.value)} placeholder="Tìm mã batch, loại đồ, loại vải, nhãn..."/></label><span className="ops-list-result">{shown.length} kết quả · 6 item/trang</span></div>
      <div className="ops-list">{paged.map(batch=><article className="ops-card" key={batch.id} role="button" tabIndex={0} onClick={()=>open(batch)} onKeyDown={e=>e.key==='Enter'&&open(batch)}>
        ...
        <div className="ops-card-footer"><span><strong>{batch.receivedItemCount??batch.expectedItemCount}</strong> item · <strong>{batch.receivedWeightKg??batch.expectedWeightKg}</strong> kg</span><span className="ops-card-action">Chi tiết <ArrowRight size={14}/></span></div>
      </article>)}{!loading&&!shown.length&&<div className="ops-empty"><Warehouse size={36}/><h4>Không có batch phù hợp</h4><p>Thử thay đổi từ khóa hoặc chọn hàng đợi khác.</p></div>}</div>
      {shown.length>pageSize&&<div className="ops-list-pagination"><Pagination currentPage={page} totalPages={totalPages} onPageChange={setPage}/></div>}
    </section>
  </div>;
}
