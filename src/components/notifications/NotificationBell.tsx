...
export default function NotificationBell() {
  ...
  const select = async (item: AppNotification) => {
    if (!item.isRead) {
      ...
      setItems((v) => v.map((x) => (x.id === item.id ? { ...x, isRead: true } : x)));
      setUnread((v) => Math.max(0, v - 1));
    }
    setOpen(false);
    const target =
      item.type === 'DonationRequestCreated' && item.donationRequestId
        ? `/manager/dispatch?requestId=${item.donationRequestId}`
        : item.targetUrl;
    if (target) navigate(target);
  };
  return (
    <div className="app-notification" ref={rootRef}>
      <button
        type="button"
      ...
      {open && (
        <section className="app-notification-panel">
          ...
          <div className="app-notification-list">
            {!items.length && <p className="app-notification-empty">Chưa có thông báo.</p>}
            {items.map((item) => (
              <button
                type="button"
                key={item.id}
